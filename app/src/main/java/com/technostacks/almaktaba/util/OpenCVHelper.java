...
public class OpenCVHelper {
    ...
    public static Observable<MatData> getMonochromeMat(MatData matData) {
        return Observable.create(sub -> {
            ...
            Imgproc.threshold(edgeMat, matData.monoChrome, 20, 255, Imgproc.THRESH_BINARY);
            //double otsu_thresold = Imgproc.threshold(edgeMat, matData.monoChrome, 0, 255, Imgproc.THRESH_BINARY + Imgproc.THRESH_OTSU);
            //Log.e("otsu_thresold", otsu_thresold * 0.5 + " - " + otsu_thresold);
            //Imgproc.threshold(edgeMat, matData.monoChrome, otsu_thresold * 0.3, otsu_thresold * 2, Imgproc.THRESH_BINARY);
            ...
            for (int i = 0; i < contours.size(); i++) {
                ...
                if (points.size() == 4 && mincos >= -0.3 && maxcos <= 0.5) {
                    for (int j = 0; j < points.size(); j++) {
                        Imgproc.circle(matData.resizeMat, points.get(j), 6, new Scalar(255, 0, 0), 6);
                    }
